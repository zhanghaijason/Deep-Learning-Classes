...
def conv_forward_naive(x, w, b, conv_param):
    ...
    #get the size of output image at a certain channel

    # reported as float datatype if not force int 
    outH = 1 + int((H-HH+2*pad)/stride)
    outW = 1 + int((W-WW+2*pad)/stride)

    # pad the all the input data, pad 0 in N, 0 in H, pad pad in H, pad pad in W
    x_pad = np.pad(x, ((0,0), (0,0),(pad,pad),(pad,pad)), 'constant')
    pad_H = x_pad.shape[2]
    pad_W = x_pad.shape[3]
    # output shape
    out = np.zeros((N, F, outH, outW))
    # reshape the filters to do matrix multiplication for the conv
    w_row = w.reshape(F, C*HH*WW)
    x_col = np.zeros((C*HH*WW, outH*outW))

    #do the loop to convolve for each image in x_pad
    for index_i in range(N):
      neuron = 0
      for i in range(0, pad_H-HH+1, stride):
        for j in range(0, pad_W-WW+1, stride):
          x_col[:, neuron] = x_pad[index_i, :, i:i+HH, j:j+WW].reshape(C*HH*WW)
          neuron += 1
      out[index_i] = (w_row.dot(x_col)+b.reshape(F,1)).reshape(F, outH, outW)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    pad = conv_param['pad']
    stride = conv_param['stride']
    # get dimension of dout
    N, F, outH, outW = dout.shape
    F, C, HH, WW = w.shape
    # initialize dx dw db
    H = (outH-1)*stride+HH-2*pad
    W = (outW-1)*stride+WW-2*pad

    padH, padW = x_pad.shape[2], x_pad.shape[3]
    ...
    for index in range(N):
      # calculate it row by row
      out_col = dout[index].reshape(F, outH*outW)
      dx_out = w_row.T.dot(out_col)
      dx_cur = np.zeros((C, padH, padW))
      db += np.sum(out_col, axis = 1)
      neuron = 0
      for i in range(0, padH-HH+1, stride):
        for j in range(0, padW-WW+1, stride):
          dx_cur[:, i:i+HH, j:j+WW] += dx_out[:,neuron].reshape(C, HH, WW)
...
def max_pool_forward_naive(x, pool_param):
    ...
    #print(H)
    #print(W)
    #assert((H-pool_height)%stride)
    #assert((W-pool_width)%stride)
      ...
      for c in range(C):
        ...
        for i in range(0, H-pool_height+1,stride):
          for j in range(0, W-pool_width+1, stride):
            temp = x[n,c,i:i+pool_height, j:j+pool_width]
            out[n,c,neuron] = np.max(temp)
            neuron += 1

    out = out.reshape([N, C, outH, outW])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
