...
def rnn_step_backward(dnext_h, cache):
    ...
    dtan_h = 1-tanh**2 # dtanhx/dx = sech^2x = 1-tanh^2x   (N,H)
    dnext_tan_h = dnext_h*dtan_h  #(N,H)
    dx = dnext_tan_h.dot(Wx.T)  #(N,D)
    dWx = x.T.dot(dnext_tan_h)     #(D,H)
    dprev_h = dnext_tan_h.dot(Wh.T)  #(N,H) 
    dWh = prev_h.T.dot(dnext_tan_h) #(H,H)
    db = dnext_tan_h.sum(axis = 0)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      x_temp = x[:, i, :]
      h0, cache_h = rnn_step_forward(x_temp, h0, Wx, Wh, b)
      h[:,i,:] = h0
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    o = sigmoid(A[:, 2*H:3*H])
    g = np.tanh(A[:, 3*H:4*H])
    B = np.concatenate((i, f, o, g), axis=1)
    next_c = f*prev_c+i*g
    next_c_tanh = np.tanh(next_c)
    next_h = o*next_c_tanh
    cache = (x, prev_h, prev_c, Wx, Wh, b, B, next_c_tanh)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
      h[:,i,:], prev_c, cache_c = lstm_step_forward(x[:, i, :], prev_h, prev_c, Wx, Wh, b)
      prev_h = h[:, i, :]
